...
configure_claude() {
  ...
  # Refero: OAuth-capable remote MCP, no token stored in repo/config by this bootstrap.
...
configure_codex() {
  ...
  if ! codex mcp get refero >/tmp/codex-refero.$$ 2>/dev/null || \
    ...
    codex mcp remove refero >/dev/null 2>&1 || true
...
EOF
  ...
  log "bootstrap complete; run scripts/validate-capabilities.sh"
  if [[ -z "${API_KEY_21ST:-}" ]]; then
    warn "21st is configured but API_KEY_21ST is not present in this shell."
  fi
  log "Refero OAuth may still require: claude /mcp or 'codex mcp login refero'."
}
